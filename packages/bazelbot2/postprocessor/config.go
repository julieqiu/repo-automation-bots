...
func loadConfigs(ppcPath, obcPath string) (_ *postProcessorConfig, _ *owlBotConfig, err error) {
	defer derrors.Wrap(&err, "loadConfigs(%q, %q)", ppcPath, obcPath)
	var ppc postProcessorConfig

	slog.Info(fmt.Sprintf("loadConfigs(%q, %q)", ppcPath, obcPath))
